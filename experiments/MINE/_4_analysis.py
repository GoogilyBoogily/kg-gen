...
def extract_graph_statistics(kg_data: dict) -> dict:
    ...
    stats_dict["entities_to_relations_ratio"] = (
        stats_dict["num_entities"] / stats_dict["num_relations"] if stats_dict["num_relations"] > 0 else 0
    )
    ...
    if G.number_of_nodes() > 0:
        ...
        stats_dict["isolated_nodes_ratio"] = (
            stats_dict["num_isolated_nodes"] / G.number_of_nodes() if G.number_of_nodes() > 0 else 0
        )
...
def collect_all_data(results_dir: Path) -> pd.DataFrame:
    ...
    model_dirs = [d for d in results_dir.iterdir() if d.is_dir() and d.name != "comparisons"]
...
def plot_top_correlations(corr_df: pd.DataFrame, df: pd.DataFrame, output_file: Path, top_n=9):
    ...
    for idx, metric in enumerate(top_metrics):
        ...
        sns.regplot(data=df, x=metric, y="accuracy", ax=ax, scatter_kws={"alpha": 0.5}, line_kws={"color": "red"})
        ...
        ax.set_title(
            f"{metric}\nSpearman r={corr_info['spearman_r']:.3f} (p={corr_info['spearman_p']:.3e})", fontsize=10
        )
    ...
    plt.suptitle("Top Graph Metrics Correlated with Accuracy", fontsize=16, fontweight="bold", y=0.995)
    plt.tight_layout()
    plt.savefig(output_file, dpi=300, bbox_inches="tight")
...
def plot_model_comparison(df: pd.DataFrame, output_file: Path):
    ...
    metrics = ["accuracy", "num_entities", "num_relations", "num_unique_relation_types", "avg_degree", "density"]
    ...
    plt.suptitle("Model Comparison: Key Graph Metrics", fontsize=16, fontweight="bold")
    plt.tight_layout()
    plt.savefig(output_file, dpi=300, bbox_inches="tight")
...
def write_comprehensive_report(df: pd.DataFrame, corr_df: pd.DataFrame, output_file: Path):
    ...
    with open(output_file, "w") as f:
        ...
        model_stats = (
            df.groupby("model")
            .agg({"accuracy": "mean", "num_entities": "mean", "num_relations": "mean", "avg_degree": "mean"})
            .reset_index()
        )
        ...
        for _, row in model_stats.iterrows():
            f.write(
                f"{row['model']:<45} {row['count']:<6.0f} {row['accuracy']:<8.2f} "
